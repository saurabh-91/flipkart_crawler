...
// ######################################### user query search function ###################################################################### 
    public function user_query_search($user_query_string)
    {
        ...
        return $this->get_results($retDoc);
    }
// ###########################################################################################################################################
...
// ############################################## filtered query (for filtering user search) #################################################
    public function filtered_query($selected_brand_name, $ini_query, $selected_price_range, $ini_brand_size, $page_no)
    {
       ...
       // $os_filter 		   = $this->os_filter_builder($selected_os_name); 		// call os 	  filter builder
        $initial_query     = $this->initial_query_builder($ini_query);				// call initial search builder
        $aggregation_array = $this->aggregations_builder($ini_brand_size, $ini_query);// call aggregations   builder
